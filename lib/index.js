...
exports.static = function static(directory, options) {
    ...
    options.babelminifyjs = Object.assign({
        enabled: true,
        sourceMaps: true
    }, options.babelminifyjs);
    ...
    function cacheFile(filePath, stat) {
        ...
        if (ext === '.css') {
            ...
            if (options.uglifyjs.enabled && !shouldIgnore(relativeUrl, options.uglifyjs.ignore)) {
            ...
            } else if (options.babelminifyjs.enabled && !shouldIgnore(relativeUrl, options.babelminifyjs.ignore)) {
                  var babelminifyjsOptions = JSON.parse(JSON.stringify(options.babelminifyjs));
                  delete babelminifyjsOptions.enabled;
                  delete babelminifyjsOptions.ignore;

                  console.log(babelminifyjsOptions);

                  var result = babelMinify(data, undefined, {sourceMaps: true});

                  if (result.error) {
                      console.warn(`babel-minify skipping ${filePath}:\n    ${JSON.stringify(result.error)}`);
                  } else {
                      // console.log('data', result);
                      data = result.code;
                  }
