...
exports.static = function static(directory, options) {
    ...
    if (!('compress' in options.uglifyjs)) {
        options.uglifyjs.compress = {sequences: false};
    }
    ...
    if (!('ext' in options.posthtml)) {
        options.posthtml.ext = '.html';
    }
    ...
    function cacheFile(filePath, stat) {
        ...
        if (!stat.isDirectory()) {
            ...
            } else if (options.posthtml.enabled && ext === options.posthtml.ext && !shouldIgnore(relativeUrl, options.posthtml.ignore)) {
                // PostHTML
                try {
                    data = posthtml(options.posthtml.plugins).process(fs.readFileSync(filePath).toString(), {sync: true}).html;
                } catch (e) {
                    console.warn(util.format('PostHTML skipping %s:\n %s (line %s, col %s, pos %s)', filePath, e.message, e.line, e.col, e.pos));
                }
            }
            ...
            if (ext === '.html' || ext === '.htm') {
                ...
                // Support `/file` to serve `/file.html` and `/file.htm`.
                aliasesToAdd.push(relativeUrl.replace(/.html?$/, ''));

                if (relativeUrl === '/index.html' || relativeUrl === '/index.htm') {
                    // Support `/` to serve `/index.html` or `/index.htm`.
                    aliasesToAdd.push('/');
                } else if (basename === 'index.html' || basename === 'index.htm') {
                    // Support `/dir/` to serve `/dir/index.html` or `/dir/index.htm`.
                    aliasesToAdd.push(path.dirname(relativeUrl) + '/');
                }

                if (!filesAliases[relativeUrl]) {
                    filesAliases[relativeUrl] = [];
                }

                aliasesToAdd.forEach(function (aliasUrl) {
                    files[aliasUrl] = files[relativeUrl];
                    filesAliases[relativeUrl].push(aliasUrl);
                });
            }
    ...
    function liveReload(filePath) {
        if (options.livereload && options.livereload.enabled && options.livereload.listener) {
            options.livereload.listener.changed(filePath);
        }
    }
    ...
    if (options.watch.enabled) {
        watch.createMonitor(directory, {interval: .005}, function monitorFiles(monitor) {
            ...
            monitor.on('removed', function removeFile(f, stat) {
                ...
                var relativeUrl = toRelativeUrl(compiledPath);

                liveReload(compiledPath);

                delete files[relativeUrl];
                (filesAliases[relativeUrl] || []).forEach(function (url) {
                    delete files[url];
                });
            });
        });
